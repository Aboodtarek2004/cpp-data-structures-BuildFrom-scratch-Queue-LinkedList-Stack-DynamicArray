...
- Add bounds checks and safer return handling for invalid index access
- Keep a tail pointer in `clsDblLinkedList` to make queue `push` truly `O(1)`
- Improve `clsDynamicArray` with capacity growth strategy (vector-like amortized append)
- Add unit tests for empty-container edge cases
